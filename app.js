/* ============================================================
   StudyMind — app.js  (완전 동작 버전)
   Vanilla ES6+ / LocalStorage / SheetJS(Excel)
   ------------------------------------------------------------
   기능
   ① AI 학습 목표 분해 (현재: 로컬 생성기 / 실제 API 훅 포함)
   ② 학습 대시보드 (스트릭·히트맵·진도율·주간추이)
...
(function () {
  ...
  // 연속 학습일(스트릭): 오늘(또는 어제)부터 뒤로 "완료 과제 ≥1"인 날 연속
  function calcStreak() {
    let streak = 0;
    let cur = new Date();
  ...
  const TASK_TEMPLATES = {
    language: [
      ['LC Part {n} 문제 {q}개 풀기', 30], ['핵심 단어 {q}개 암기', 20],
      ['RC 빈칸추론 {q}문제 풀기', 25], ['문법 정리 Chapter {n}', 20],
      ['오답노트 정리', 15], ['받아쓰기 {q}문장 연습', 20], ['독해 지문 {q}개 분석', 25],
    ],
    coding: [
      ['백준 실버 {q}문제 풀기', 60], ['{topic} 개념 복습', 20],
      ['알고리즘 유형 정리', 30], ['프로그래머스 Lv.{n} {q}문제', 45],
      ['오답 코드 리팩터링', 25], ['{topic} 예제 코드 작성', 30],
    ],
    certificate: [
      ['기출문제 {q}개 풀기', 40], ['이론 Chapter {n} 정리', 30],
      ['오답노트 복습', 20], ['핵심 개념 {q}개 암기', 25], ['모의고사 1회분', 50],
    ],
    etc: [
      ['학습 자료 {q}p 읽기', 30], ['핵심 개념 복습', 20],
      ['연습 문제 {q}개 풀기', 30], ['정리 노트 작성', 20], ['복습 퀴즈 {q}문항', 15],
    ],
  };
  const CODING_TOPICS = ['자료구조', '정렬', 'DP', '그래프', '문자열', '완전탐색', '이분탐색', '그리디'];
  ...
  function renderToday() {
    el('today-date').textContent = fmtKDate(new Date());
    el('today-date').setAttribute('datetime', todayISO());
    ...
    el('today-footer-info').innerHTML = `
      <p id="today-estimated-time">⏰ 오늘 예상 학습 시간: ${fmtMinutes(totalMin)}</p>
  ...
  function onTaskToggle(e) {
    ...
    if (el('screen-today').classList.contains('is-active')) {
      ...
      // 전체 완료 시 축하 모달
      if (total > 0 && done === total && cb.checked) showCelebration(done);
    }
  ...
  function showCelebration(count) {
    const streak = calcStreak();
    el('celebration-streak').textContent = `🔥 ${streak}일 연속 학습 중`;
  ...
  function onDeleteGoal() {
    const g = goalById(currentGoalId); if (!g) return;
    if (!confirm('정말 삭제하시겠어요? 모든 과제와 기록이 사라집니다.')) return;
  ...
  function notifyTick() {
    ...
    if (s.studyAlarm && s.alarmDays.includes(now.getDay()) && hhmm === s.alarmTime) {
      ...
      new Notification('📚 StudyMind', {
        body: t ? `오늘 "${t.text}"${g ? ' ['+g.name+']' : ''} 이(가) 기다리고 있어요!` : '오늘의 학습을 시작해볼까요?',
      });
      lastNotified = stamp;
    }
    // 미완료 리마인드
  ...
  function exportExcel() {
    ...
    const recRows = dateSet.map(d => {
      ...
      return { 날짜: d, 완료과제수: st.done, 전체과제수: st.total, 학습시간분: min };
    });
    ...
    const name = `StudyMind_backup_${todayISO().replace(/-/g, '')}.xlsx`;
  ...
  function init() {
    ...
    // 콘솔 디버깅용
    window.StudyMind = { state, save, navigateTo, get: () => state };
  }
